...
class Company extends Client{
    ...
    public function setCommercialreg(string $commercialreg):int {
        ...
        if($error == 0) $this->commercialreg =$commercialreg;
        return $error;
    }

    public function setType(string $type): int {
        $error = Checker::StringValidator($type, 2);
        if($error == 0) $this->type = $type;
        return $error;
    }
    
    public function setEmployees(int $employees): int {
        $error = Checker::NumberValidator($employees, 2);
        if($error == 0) $this->employees = $employees;
        return $error;
    }
